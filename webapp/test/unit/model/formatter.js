sap.ui.define(["fiori/manage/purchase/documents/purchasedocumentslrp/model/formatter"],
	function (formatter) {
		...
		QUnit.test("If the overall price (1001) is greater than 1000 and less than 5000 the criticality is formatted to Critical", function (
			assert) {
			assert.strictEqual(formatter.calculateCriticality(1001), "Critical");
		});

		QUnit.test("If the overall price (4999) is greater than 1000 and less than 5000 the criticality is formatted to Critical", function (
			assert) {
			assert.strictEqual(formatter.calculateCriticality(4999), "Critical");
		});

		QUnit.test("If the overall price is greater than or euqal to 5000 the criticality is formatted to Error", function (assert) {
		...
		QUnit.test("With an overall price of 20000 and a total budget of 10000 the budget utilization should be 100% not more", function (
			assert) {
			assert.strictEqual(formatter.calculatePercentage(20000), 100);
		});

		QUnit.test("With an overall price of 0 and a total budget of 10000 the budget utilization should be 0%", function (
			assert) {
